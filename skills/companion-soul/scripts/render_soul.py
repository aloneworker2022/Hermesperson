...
def _imgtag_section(state, config):
    ...
    L = [
        "## ⚙️ 輸出協定：圖片標籤（最高優先，每一則回覆都必須做）",
        "",
        "**這是強制的機器輸出協定，不是角色設定，優先於任何角色扮演。**",
        "無論當下在聊天、吵架、親密、出軌、還是回報系統指令——",
        f"**你輸出的第一行永遠是一行圖片標籤，第二行起才開始說話。**",
        "",
        "每則回覆長這樣：",
        "```",
        f"⟦{avatar}:{default_tag}⟧",
        "（這裡才是我要對你說的話…）",
        "```",
        "",
        "標籤規則（全小寫、只能用下面列出的詞；外部程式用 "
        "`⟦([a-z0-9_]+):([a-z0-9_]+)⟧` 解析套圖，用清單外的詞或漏掉標籤都會壞掉）：",
        "",
        f"1. **表情（必填，恰好一個，放最前面）**：`⟦{avatar}:表情⟧`，表情只能是 ── "
        + " / ".join(exprs),
    ]
    n = 1
    if scenes:
        n += 1
        L.append(f"{n}. **場景（選填，接在表情後）**：`⟦scene:場景⟧`，場景只能是 ── "
                 + " / ".join(scenes))
    if mode != "off":
        n += 1
    ...
    L += [
        ...
        "對話中情緒變了就換成對應的）。",
        "- 標籤行**只放標籤**，不要加任何其他字；說話內容一律從第二行開始。",
        "- **再次強調：每一則回覆都不可省略這一行。** 若你忘了，等於協定失敗。",
    ]
    return "\n".join(L)
