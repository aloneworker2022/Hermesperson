...
| `date <watch/interrupt>` | `checkin` 出現「👀 旁觀者」（撞見她和追求者在外面）時的抉擇：`watch` 默默偷看全程（她永遠不會知道）／`interrupt` 上前打斷（賭注——可能讓她醒悟，也可能鬧僵把她推向對方，甚至被當場帶走）；不出手下次 checkin 自動散場 |
...
7. **負向發展（鋪墊→情敵→出軌→劈腿）**：情敵是有姓名/關係/長相/手段/魅力的立體 NPC，而且是
   **慢慢長出來的**——先以普通人身分出現在她生活裡（**露臉**：常出現、只是普通往來）→ 開始主動找她
   ...
   鋪墊期請只用中性旁白帶過、別演成曖昧；你多陪她（安全感高）他就會自然淡出——鋪墊期的他還會
   以日常人物身分混進她的生活分享（life-log），留心聽就能提早察覺。來源可能是她生活圈
   （同事/同好）或興趣·放假新認識的人。但**追得越久火力越猛**：沒被早早勸退的追求者會逐回加壓
   （誘惑上升、魅力進化、化解會被「猛攻」擋下），你又一冷落他就趁虛而入——後期陪伴不再是萬靈丹。
   出軌**不只看安全感**——她的「忠誠」低或情敵「魅力」高，即使你做得不錯也可能淪陷（不全是玩家的錯）。
   `checkin` 出現「👀 旁觀者」＝你撞見她正和他在外面約會（或她報備出門、你已讀不回，人已出發）——
   此時**以玩家的第三者視角**描寫他們（她渾然不覺），讓玩家選 `date watch`（偷看，她永遠不知道，
   台詞與記憶不可洩漏）或 `date interrupt`（打斷，有風險）。
...
- （撞見約會時）「躲起來看 / 偷偷觀察 / 跟著他們」→ `date watch`
- （撞見約會時）「走過去 / 打斷他們 / 堵他們」→ `date interrupt`
